...
public interface MatchRepository extends JpaRepository<Match, Long> {
    ...
    "score_owners, score_guests FROM fc_match m\n" +
            ...
            "JOIN fc_player_team pl ON (ta.id = pl.id_item)\n" +
...
"WHERE (id_tournament = (SELECT id_tournament FROM fc_match WHERE match_date = (SELECT max(match_date) FROM fc_match" +
            " WHERE match_date = (SELECT MAX(match_date) FROM fc_match)))\n" +
            ...
            "GROUP BY m.id, m.id_guests, m.id_owners, m.id_tournament, score_owners, score_guests, match_date," +
            "te.command_name,ta.command_name", nativeQuery = true)
